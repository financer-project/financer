...
export function buildPrismaWhere<T, W extends Record<string, unknown> = Record<string, unknown>>(
    ...
    if (search && Array.isArray(search.fields) && search.fields.length > 0) {
        ...
        if (term) {
            ...
            if (or.length > 0) andClauses.push({ OR: or } as unknown as W)
        }
    }
...
function toUrlSearchParams(input: URLSearchParams | Record<string, string | string[] | undefined> | null): URLSearchParams {
    if (!input) return new URLSearchParams()
    if (input instanceof URLSearchParams) return input
    const entries: Array<[string, string]> = []
    for (const [k, v] of Object.entries(input)) {
        if (v === undefined) continue
        if (Array.isArray(v)) entries.push(...v.map((x) => [k, x] as [string, string]))
